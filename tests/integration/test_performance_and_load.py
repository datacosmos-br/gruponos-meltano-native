...
class TestConnectionPerformance:
    ...
    def test_connection_pool_stress_test(
        ...
        config_data = performance_config.model_dump()
        config_data.update({
            "connection_pool_size": 5,
            "connection_timeout": 5,
        })
        stress_config = OracleConnectionConfig(**config_data)
        managers = []
        ...
        try:
            ...
            for i, manager in enumerate(managers):
                with contextlib.suppress(Exception):
                    result = manager.fetch_one("SELECT ? FROM DUAL", (i + 1,))
...
class TestDDLGenerationPerformance:
    ...
    def test_complex_ddl_generation_performance(
        ...
        for num_columns in schema_complexities:
            ...
            for i in range(num_columns):
                ...
                if col_type == "string":
                    ...
                    properties[f"column_{i:03d}"]["minimum"] = "0"
                    properties[f"column_{i:03d}"]["maximum"] = "10000"
        ...
        # DDL quality checks
        assert largest_schema["ddl_size"] > 5000, "DDL should be substantial for complex schema"
        assert largest_schema["num_indexes"] >= 0, "Should report index count"

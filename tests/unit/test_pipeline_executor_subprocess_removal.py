...
class TestPipelineExecutorNoSubprocessImports:
    ...
    def test_no_subprocess_import(self) -> None:
        ...
        executor_path = Path(
            __file__
        ).parent.parent.parent / "src/gruponos_meltano_native/core/pipeline_executor.py"
    ...
    def test_no_subprocess_method_calls(self) -> None:
        ...
        ), "subprocess.run() still present in file"
        assert (
            "subprocess.Popen" not in source
        ), "subprocess.Popen() still present in file"
        assert (
            "subprocess.TimeoutExpired" not in source
    ...
    def test_get_job_status_uses_run_external_command(self) -> None:
        ...
        for node in ast.walk(tree):
            if (
                ...
                and node.name == "get_job_status"
            ):
                method_code = ast.unparse(node)
                assert (
                    "FlextUtilities.run_external_command" in method_code
...
class TestPipelineExecutorErrorHandling:
    ...
    def test_error_message_detection_instead_of_exceptions(self) -> None:
        ...
        assert (
            "exec_result.is_failure" in source
        ), "No failure checking on FlextResult"
        assert (
            "exec_result.error" in source
        ), "No error message access from FlextResult"
    ...
    def test_json_decode_error_handling(self) -> None:
        ...
        for node in ast.walk(tree):
            if (
                isinstance(node, ast.FunctionDef)
                and node.name in ["get_job_status", "list_jobs", "list_pipelines"]
            ):
                # These methods parse JSON
                assert (
                    "json.loads" in ast.unparse(node)
...
class TestPipelineExecutorModuleStructure:
    ...
    def test_module_can_be_imported(self) -> None:
        ...
        try:
            ...
            if "subprocess" in str(e):
                pytest.fail(
                    f"Module import failed due to subprocess issue: {e}"
                )
            raise
    ...
    def test_executor_class_has_required_methods(self) -> None:
        ...
        for method_name in required_methods:
            assert hasattr(
                MeltanoPipelineExecutor, method_name
    ...
    def test_flextlogger_initialized_correctly(self) -> None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == "__init__":
                ...
                assert (
                    "self.logger" in init_code
                ), "__init__ does not initialize self.logger"
                assert (
                    "FlextLogger.get_logger" in init_code
...
class TestSprintTwoPatternConsistency:
    ...
    def test_uses_timeout_parameter_correctly(self) -> None:
        ...
        assert "timeout=30.0" in source, (
            "Timeouts not specified as float (e.g., 30.0)"
        )
    ...
    def test_wrapper_attribute_access(self) -> None:
        ...
        assert "wrapper.stdout" in source, (
            "Does not access wrapper.stdout"
        )
        assert "wrapper.stderr" in source, (
            "Does not access wrapper.stderr"
        )
        assert "wrapper.returncode" in source, (
            "Does not access wrapper.returncode"
        )

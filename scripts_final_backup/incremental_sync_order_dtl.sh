#!/bin/bash
# Incremental sync for order_dtl entity (run every minute)

cd /home/marlonsc/flext/gruponos-meltano-native

# Create incremental config with last 7 days
cat > tap_config_incremental_order_dtl.json << EOF
{
...
python tap_oracle_wms_wrapper.py --config tap_config_incremental_order_dtl.json --catalog catalog_test.json | \

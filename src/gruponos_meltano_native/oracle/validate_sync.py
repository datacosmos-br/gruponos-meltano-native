...
def _get_table_details(cursor: Any, table_name: str) -> dict[str, str | int | None]:
    ...
    try:
        ...
        try:
            ...
            # If MOD_TS doesn't exist, try without it
            if "ORA-00904" in str(e):  # invalid identifier
                try:
                    cursor.execute(
                        f'SELECT COUNT(DISTINCT "ID") as unique_ids '                        f'FROM "OIC"."{table_name}"',
                    )
                    (unique_ids,) = cursor.fetchone()
                    details["unique_ids"] = unique_ids or 0
                except Exception:
                    logger.exception("Error getting unique IDs for table %s", table_name)
            else:
                logger.exception("Error getting table details for %s", table_name)
        # Check for duplicates
        try:
            cursor.execute(
                f"SELECT COUNT(*) FROM ("                f'SELECT "ID" FROM "OIC"."{table_name}" '
            ...
            if "ORA-00904" in str(e):  # ID column doesn't exist
                details["duplicates"] = 0
            else:
                logger.exception("Error checking duplicates for table %s", table_name)
    except Exception:
